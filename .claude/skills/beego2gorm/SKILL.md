---
name: beego2gorm
description: 将 Beego ORM 代码迁移为 GORM 代码。当用户想要将文件、目录或模块中的 Beego ORM 用法转换为 GORM 时使用。
license: MIT
metadata:
  author: ai-coding-best-practices
  version: "1.0"
---

将指定文件或目录中的 Beego ORM 代码迁移为 GORM，遵循项目规范。

**输入**：用户的请求应包含要转换的文件路径、目录路径或模块名。

- 文件路径：`internal/template/internal/repository/template_component_repo.go`
- 目录路径：`internal/template/internal/repository/`
- 模块名：`template`
- 无参数：交互式询问目标
...
// GORM — 根据 Model 对应的数据库选择正确的实例获取函数
db := register.GetGOrmDefault()
```
